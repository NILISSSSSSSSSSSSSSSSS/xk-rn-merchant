...
class ContractScreen extends PureComponent {
    ...
    componentDidMount() {
        ...
        const contractConfigKey = `${store.user.merchant.filter((item)=>item.merchantType==merchantType)[0].agreement}_LEAGUE_CONTRACT`;
    ...
    render() {
        ...
        const { name, page, merchantType,auditStatus,route,familyUp, agreementUri } = this.state;
        return (
...
{/*
                <TouchableOpacity
                    onPress={() => {
                            navigation.navigate('Appoint', navigation.state.params || {});
                    }}
                    style={styles.btn}
                >
