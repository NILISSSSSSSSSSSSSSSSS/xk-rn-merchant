...
class SystemMessage extends PureComponent {
    ...
    renderSystemItem = item => (
      ...
      >
        <Text style={[styles.title, styles.itemLeft]}>
          {item.msgContent}
          {' '}
        </Text>
      ...
      <View style={styles.item}>
        <View style={styles.itemTop}>
          <View style={[styles.itemLeft, { flexDirection: 'row', flexWrap: 'wrap' }]}>
            ...
            <TouchableOpacity onPress={() => imJudge(item.extras)}>
    ...
    renderSpecialItem = ({ item }) => (
      <View style={[styles.item, { padding: 0 }]}>
        <View style={[styles.itemTop, {
          backgroundColor: '#FAFAFA', height: 38, alignItems: 'center', width: width - 20, paddingHorizontal: 15,
        }]}
        >
          <Text style={[styles.title, styles.itemLeft]}>
            {item.msgContent}
。
    ...
    renderItem = (item) => {
      ...
      const defaultSource = 'https://ss0.bdstatic.com/70cFuHSh_Q1YnxGkpoWK1HF6hhy/it/u=180635262,3469104988&fm=27&gp=0.jpg';
                    ...
                    code == '003'
                      ...
                      : (
                        <SwipeListView
                            useFlatList
                            style={styles.listview}
                            renderItem={(data, secId, rowId, rowMap) => renderItem(data.item)
                            }
                            store={{
                              ...longLists[listName],
                              page: longLists[listName] && longLists[listName].listsPage || 1,
