...
class ActionSheet extends React.Component {
    ...
    constructor(props) {
        ...
        this._throttleHide = throttle(this._hideSheet,500);
        this.hide = (index)=> this._throttleHide(() => {
          this.props.onPress(index)
          this.handler && this.handler(index);
        });
    }
    ...
    _showSheet = () => {
        ...
        Platform.OS=='android' ? params.useNativeDriver=true:null;
        Animated.timing(this.state.sheetAnim, params).start()
    }
    ...
    _hideSheet(callback) {
        ...
        let hideModal = ()=>{
            this.callback = Platform.OS=='ios'?callback : ()=>{};
            let newCallback = ()=>Platform.OS=='android'?callback() : ()=>{};
            this.setState({ visible: false }, newCallback);
        }
        Animated.timing(this.state.sheetAnim, params).start(hideModal);
    }
    ...
    _createButton(title, index) {
        ...
        const { buttonUnderlayColor, cancelButtonIndex, destructiveButtonIndex } = this.props
        const tintColor = '#222222'
