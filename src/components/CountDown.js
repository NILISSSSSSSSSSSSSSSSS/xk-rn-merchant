...
class CountDown extends Component {
    ...
    render() {
      const { type, delay, label } = this.props;
      ...
      // console.log(this.leadingZeros(countDown.sec))
      let second = this.leadingZeros(countDown.sec);
      if (delay > 60) {
      ...
      let _hours = countDown.days * 24 + countDown.hours;
      _hours = _hours < 10 ? `0${_hours}` : _hours;
      return (
        <React.Fragment>
          {
                    type === 'verCode'
                      ? <Text style={styles.verStyle}>{second}</Text>
                      : type !== 'orderApply'
                        ? (
                          <View style={this.props.containerStyle}>
                            {(countDown.days > 0) ? (
                              <View style={{ flexDirection: 'row' }}>
                                ...
                                <Text style={this.props.secondColonStyle}>{days}</Text>
                              </View>
                            ) : null}
                        ...
                        : (
                          <View style={this.props.containerStyle}>
                            <Text style={[this.props.hoursStyle, { paddingRight: 5 }]}>{label || ''}</Text>
                            <Text style={this.props.hoursStyle}>
                              {_hours}
