...
- (void)viewDidLoad {
  ...
  [self.navigationController.navigationBar setBarTintColor:XKMainTypeColor];
  ...
  [back setImage:[UIImage imageNamed:@"live_close"] forState:UIControlStateNormal];
  [back addTarget:self action:@selector(back) forControlEvents:UIControlEventTouchUpInside];
  UIBarButtonItem *bacItem = [[UIBarButtonItem alloc] initWithCustomView:back];
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
  ...
  if (metadataObjects.count>0) {
    ...
    EXECUTE_BLOCK(_resultBlock,str_code);
    [self dismissViewControllerAnimated:YES completion:nil];
...
- (void)codeResultXC_url:(NSString *)str {
  EXECUTE_BLOCK(_resultBlock,str);
  [self dismissViewControllerAnimated:YES completion:nil];
