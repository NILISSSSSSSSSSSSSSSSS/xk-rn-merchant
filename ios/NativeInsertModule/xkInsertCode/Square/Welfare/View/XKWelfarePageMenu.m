//
//  XKWelfarePageMenuPageMenu.m
//  XKWelfarePageMenuPageMenu
//
...
@interface XKWelfarePageMenuPageMenuLine : UIImageView
@property (nonatomic, copy) void(^hideBlock)(void);

@end

@implementation XKWelfarePageMenuPageMenuLine
...
@interface XKWelfarePageMenuItem : UIButton

@property (nonatomic, assign) CGFloat imageRatio;
@property (nonatomic, assign) XKWelfarePageMenuItemImagePosition imagePosition;
@end

@implementation XKWelfarePageMenuItem
...
- (void)initialize {
    _imageRatio = 0.5;
    _imagePosition = XKWelfarePageMenuItemImagePositionDefault;
}
...
- (CGRect)imageRectForContentRect:(CGRect)contentRect {
    ...
    switch (self.imagePosition) {
        case XKWelfarePageMenuItemImagePositionDefault:
        case XKWelfarePageMenuItemImagePositionLeft: {
            ...
            break;
        case XKWelfarePageMenuItemImagePositionTop: {
            ...
            break;
        case XKWelfarePageMenuItemImagePositionRight: {
            _imageRatio = _imageRatio == 0.0 ? 0.5 : _imageRatio;
            CGFloat imageW =  contentRect.size.width * _imageRatio;
            CGFloat imageH = contentRect.size.height;
            ...
            break;
        case XKWelfarePageMenuItemImagePositionBottom: {
            _imageRatio = _imageRatio == 0.0 ? 2.0/3.0 : _imageRatio;
            CGFloat imageW =  contentRect.size.width;
            CGFloat imageH = contentRect.size.height * _imageRatio;
...
- (void)setImagePosition:(XKWelfarePageMenuItemImagePosition)imagePosition {
...
@interface XKWelfarePageMenu()
@property (nonatomic, assign) XKWelfarePageMenuPageMenuTrackerStyle trackerStyle;
...
@property (nonatomic, weak) XKWelfarePageMenuItem *functionButton;
@property (nonatomic, weak) CALayer *shadowLine;
@property (nonatomic, strong) NSMutableArray *buttons;
@property (nonatomic, strong) XKWelfarePageMenuItem *selectedButton;
...
@implementation XKWelfarePageMenu


#pragma mark - public

+ (instancetype)pageMenuWithFrame:(CGRect)frame trackerStyle:(XKWelfarePageMenuPageMenuTrackerStyle)trackerStyle {
    XKWelfarePageMenu *pageMenu = [[XKWelfarePageMenu alloc] initWithFrame:frame trackerStyle:trackerStyle];
    return pageMenu;
}

- (instancetype)initWithFrame:(CGRect)frame trackerStyle:(XKWelfarePageMenuPageMenuTrackerStyle)trackerStyle {
    if (self = [super init]) {
        self.frame = frame;
...
- (void)setItems:(NSArray *)items selectedItemIndex:(NSUInteger)selectedItemIndex {
    ...
    if (self.buttons.count) {
        ...
        XKWelfarePageMenuItem *selectedButton = [self.buttons objectAtIndex:selectedItemIndex];
        [self buttonInPageMenuClicked:selectedButton];

        // XKWelfarePageMenuPageMenuTrackerStyleTextZoom和XKWelfarePageMenuPageMenuTrackerStyleNothing样式跟tracker没有关联
        if (self.trackerStyle != XKWelfarePageMenuPageMenuTrackerStyleTextZoom && self.trackerStyle != XKWelfarePageMenuPageMenuTrackerStyleNothing) {
...
- (void)removeItemAtIndex:(NSUInteger)itemIndex animated:(BOOL)animated {
    if (itemIndex > self.items.count) {return;}
    // 被删除的按钮之后的按钮需要修改tag值
    for (XKWelfarePageMenuItem *button in self.buttons) {
    ...
    if (itemIndex < self.buttons.count) {
        XKWelfarePageMenuItem *button = [self.buttons objectAtIndex:itemIndex];
...
- (void)removeAllItems {
    ...
    for (int i = 0; i < self.buttons.count; i++) {
        XKWelfarePageMenuItem *button = self.buttons[i];
        [button removeFromSuperview];
    }
...
- (void)setTitle:(NSString *)title image:(UIImage *)image imagePosition:(XKWelfarePageMenuItemImagePosition)imagePosition imageRatio:(CGFloat)ratio forItemIndex:(NSUInteger)itemIndex {
    if (itemIndex < self.buttons.count) {
        XKWelfarePageMenuItem *button = [self.buttons objectAtIndex:itemIndex];
        [button setTitle:title forState:UIControlStateNormal];
        [button setImage:image forState:UIControlStateNormal];
...
- (void)setFunctionButtonTitle:(NSString *)title image:(UIImage *)image imagePosition:(XKWelfarePageMenuItemImagePosition)imagePosition imageRatio:(CGFloat)ratio forState:(UIControlState)state {
...
- (void)addButton:(NSInteger)index object:(id)object animated:(BOOL)animated {
    ...
    XKWelfarePageMenuItem *button = [XKWelfarePageMenuItem buttonWithType:UIButtonTypeCustom];
    ...
    if (self.insert && animated) { // 是插入的新按钮,且需要动画
        // 取出上一个按钮
        XKWelfarePageMenuItem *lastButton;
        if (index > 0) {
...
- (void)initialize {
    ...
    // 必须先添加分割线，再添加backgroundView;假如先添加backgroundView,那也就意味着backgroundView是XKWelfarePageMenuPageMenu的第一个子控件,而scrollView又是backgroundView的第一个子控件,当外界在由导航控制器管理的控制器中将XKWelfarePageMenuPageMenu添加为第一个子控件时，控制器会不断的往下遍历第一个子控件的第一个子控件，直到找到为scrollView为止,一旦发现某子控件的第一个子控件为scrollView,会将scrollView的内容往下偏移64;这时控制器中必须设置self.automaticallyAdjustsScrollViewInsets = NO;为了避免这样做，这里将分割线作为第一个子控件
    XKWelfarePageMenuPageMenuLine *dividingLine = [[XKWelfarePageMenuPageMenuLine alloc] init];
    dividingLine.backgroundColor = [UIColorFromRGB(0xd7d7d7) colorWithAlphaComponent:0.5];
    ...
    XKWelfarePageMenuItem *functionButton = [XKWelfarePageMenuItem buttonWithType:UIButtonTypeCustom];
...
// 按钮点击方法
- (void)buttonInPageMenuClicked:(XKWelfarePageMenuItem *)sender {
    ...
    if (self.trackerStyle == XKWelfarePageMenuPageMenuTrackerStyleTextZoom || _selectedItemZoomScale != 1) {
...
- (void)moveItemScrollViewWithSelectedButton:(XKWelfarePageMenuItem *)selectedButton {
...
// 移动跟踪器
- (void)moveTrackerWithSelectedButton:(XKWelfarePageMenuItem *)selectedButton {
    [UIView animateWithDuration:0.25 animations:^{
        [self resetSetupTrackerFrameWithSelectedButton:selectedButton];
    }];
}
...
// 功能按钮的点击方法
- (void)functionButtonClicked:(XKWelfarePageMenuItem *)sender {
...
- (void)moveTrackerWithProgress:(CGFloat)progress fromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex currentOffsetX:(CGFloat)currentOffsetX beginOffsetX:(CGFloat)beginOffsetX {
    ...
    if (self.trackerStyle == XKWelfarePageMenuPageMenuTrackerStyleLine) {
    ...
    } else if (self.trackerStyle == XKWelfarePageMenuPageMenuTrackerStyleLineAttachment) {
    ...
    } else if (self.trackerStyle == XKWelfarePageMenuPageMenuTrackerStyleTextZoom || self.trackerStyle == XKWelfarePageMenuPageMenuTrackerStyleNothing) {
        // 缩放文字
        if (_selectedItemZoomScale != 1) {
            [self zoomForTitleWithProgress:progress fromButton:fromButton toButton:toButton];
        }
    } else if (self.trackerStyle == XKWelfarePageMenuPageMenuTrackerStyleRoundedRect) {
        newCenter.x = fromButton.center.x + xDistance * progress;
...
- (void)setTrackerStyle:(XKWelfarePageMenuPageMenuTrackerStyle)trackerStyle {
    _trackerStyle = trackerStyle;
    switch (trackerStyle) {
        case XKWelfarePageMenuPageMenuTrackerStyleLine:
        case XKWelfarePageMenuPageMenuTrackerStyleLineLongerThanItem:
        case XKWelfarePageMenuPageMenuTrackerStyleLineAttachment:
            self.tracker.backgroundColor = _selectedItemTitleColor;
            break;
        case XKWelfarePageMenuPageMenuTrackerStyleRoundedRect:
        case XKWelfarePageMenuPageMenuTrackerStyleRect:
            ...
            _trackerHeight = 0;
            break;
        case XKWelfarePageMenuPageMenuTrackerStyleTextZoom:
            // 此样式下默认1.3
...
- (void)setSelectedItemIndex:(NSUInteger)selectedItemIndex {
    _selectedItemIndex = selectedItemIndex;
    if (self.buttons.count) {
        XKWelfarePageMenuItem *button = [self.buttons objectAtIndex:selectedItemIndex];
        [self buttonInPageMenuClicked:button];
    }
}

- (void)setDelegate:(id<XKWelfarePageMenuDelegate>)delegate {
    if (delegate == _delegate) {return;}
    _delegate = delegate;
    if (self.buttons.count) {
        XKWelfarePageMenuItem *button = [self.buttons objectAtIndex:_selectedItemIndex];
...
- (void)setPermutationWay:(XKWelfarePageMenuPageMenuPermutationWay)permutationWay {
...
- (void)layoutSubviews {
    ...
    for (int i= 0 ; i < self.buttons.count; i++) {
        ...
        if (button.currentTitle && !button.currentImage) {
            ...
            contentW = imageW;
        } else if (button.currentTitle && button.currentImage && (button.imagePosition == XKWelfarePageMenuItemImagePositionRight || button.imagePosition == XKWelfarePageMenuItemImagePositionLeft)) {
            contentW = textW + imageW;
        } else if (button.currentTitle && button.currentImage && (button.imagePosition == XKWelfarePageMenuItemImagePositionTop || button.imagePosition == XKWelfarePageMenuItemImagePositionBottom)) {
    ...
    XKWeakSelf(ws);
    [self.buttons enumerateObjectsUsingBlock:^(XKWelfarePageMenuItem *button, NSUInteger idx, BOOL * _Nonnull stop) {
        CGFloat setupButtonW = [[self.setupWidths objectForKey:[NSString stringWithFormat:@"%zd",idx]] floatValue];
        if (self.permutationWay == XKWelfarePageMenuPageMenuPermutationWayScrollAdaptContent) {
            buttonW = [buttonWidths[idx] floatValue];
            if (idx == 0) {
                button.frame = CGRectMake(ws.itemPadding*0.5+lastButtonMaxX, 0, buttonW, itemScrollViewH);
            } else {
                button.frame = CGRectMake(ws.itemPadding+lastButtonMaxX, 0, buttonW, itemScrollViewH);

            }
        } else if (self.permutationWay == XKWelfarePageMenuPageMenuPermutationWayNotScrollEqualWidths) {
            // 求出外界设置的按钮宽度之和
            ...
            buttonW = setupButtonW ? setupButtonW : (itemScrollViewW-ws.itemPadding*(self.buttons.count)-totalSetupButtonW)/(self.buttons.count-self.setupWidths.count);
            ...
            ws.itemPadding = diff/self.buttons.count;
            buttonW = [buttonWidths[idx] floatValue];
            if (idx == 0) {
                button.frame = CGRectMake(ws.itemPadding*0.5+lastButtonMaxX, 0, buttonW, itemScrollViewH);
            } else {
...
- (void)resetSetupTrackerFrameWithSelectedButton:(XKWelfarePageMenuItem *)selectedButton {
