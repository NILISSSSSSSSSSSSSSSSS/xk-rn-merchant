...
typedef NS_ENUM(NSInteger, XKWelfarePageMenuPageMenuTrackerStyle) {
    XKWelfarePageMenuPageMenuTrackerStyleLine = 0,                  // 下划线,默认与item等宽
    XKWelfarePageMenuPageMenuTrackerStyleLineLongerThanItem,        // 下划线,比item要长(长度为item的宽+间距)
    XKWelfarePageMenuPageMenuTrackerStyleLineAttachment,            // 下划线“依恋”样式，此样式下默认宽度为字体的pointSize，你可以通过trackerWidth自定义宽度
    XKWelfarePageMenuPageMenuTrackerStyleRoundedRect,               // 圆角矩形
    XKWelfarePageMenuPageMenuTrackerStyleRect,                      // 矩形
    XKWelfarePageMenuPageMenuTrackerStyleTextZoom NS_ENUM_DEPRECATED_IOS(2_0, 2_0, "该枚举值已经被废弃，请用“selectedItemZoomScale”属性代替"), // 缩放(该枚举已经被废弃),用属性代替的目的是让其余样式可与缩放样式配套使用，另外，XKWelfarePageMenuPageMenuTrackerStyle指的都是跟踪器样式，文字缩放和跟踪器实际上是毫无关系的，该枚举名字取的也不太合理，缩放的不单单是文字，而是整个item。如果你同时设置了该枚举和selectedItemZoomScale属性，selectedItemZoomScale优先级高于XKWelfarePageMenuPageMenuTrackerStyleTextZoom
    XKWelfarePageMenuPageMenuTrackerStyleNothing                    // 什么样式都没有
};

typedef NS_ENUM(NSInteger, XKWelfarePageMenuPageMenuPermutationWay) {
    XKWelfarePageMenuPageMenuPermutationWayScrollAdaptContent = 0,   // 自适应内容,可以左右滑动
    XKWelfarePageMenuPageMenuPermutationWayNotScrollEqualWidths,     // 等宽排列,不可以滑动,整个内容被控制在pageMenu的范围之内,等宽是根据pageMenu的总宽度对每个item均分
    XKWelfarePageMenuPageMenuPermutationWayNotScrollAdaptContent     // 自适应内容,不可以滑动,整个内容被控制在pageMenu的范围之内,这种排列方式下,自动计算item之间的间距,itemPadding属性设置无效
};

typedef NS_ENUM(NSInteger, XKWelfarePageMenuItemImagePosition) {
    XKWelfarePageMenuItemImagePositionDefault,   // 默认图片在左边
    XKWelfarePageMenuItemImagePositionLeft,      // 图片在左边
    XKWelfarePageMenuItemImagePositionTop,       // 图片在上面
    XKWelfarePageMenuItemImagePositionRight,     // 图片在右边
    XKWelfarePageMenuItemImagePositionBottom     // 图片在下面
};

@class XKWelfarePageMenu;

@protocol XKWelfarePageMenuDelegate <NSObject>
...
@interface XKWelfarePageMenu : UIView
...
/** item之间的间距,当permutationWay为‘XKWelfarePageMenuPageMenuPermutationWayNotScrollAdaptContent’时此属性无效，无效是合理的，不可能做到“不可滑动且自适应内容”然后间距又自定义，这2者相互制约 */
...
/** 排列方式 */
@property (nonatomic, assign) XKWelfarePageMenuPageMenuPermutationWay permutationWay;
...
/** 选中的item缩放系数，默认为1，为1代表不缩放，[0,1)之间缩小，(1,+∞)则放大，(-1,0)之间"倒立"缩小，(-∞,-1)之间"倒立"放大，为-1则"倒立不缩放",如果依然使用了废弃的XKWelfarePageMenuPageMenuTrackerStyleTextZoom样式，则缩放系数默认为1.3 */
...
/** 代理 */
@property (nonatomic, weak) id<XKWelfarePageMenuDelegate> delegate;
 ...
 */
- (void)setTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(XKWelfarePageMenuItemImagePosition)imagePosition imageRatio:(CGFloat)ratio forItemIndex:(NSUInteger)itemIndex;
 ...
 *  @param state            控件状态
 */
- (void)setFunctionButtonTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(XKWelfarePageMenuItemImagePosition)imagePosition imageRatio:(CGFloat)ratio forState:(UIControlState)state;
...
/* 1.让跟踪器时刻跟随外界scrollView滑动,实现了让跟踪器的宽度逐渐适应item宽度的功能;
   2.这个方法用于scrollViewDidScroll代理方法中，如
    ...
    3.如果外界设置了XKWelfarePageMenuPageMenu的属性"bridgeScrollView"，那么外界就可以不用在scrollViewDidScroll方法中调用这个方法来实现跟踪器时刻跟随外界scrollView的效果,内部会自动处理; 外界对XKWelfarePageMenuPageMenu的属性"bridgeScrollView"赋值是实现此效果的最简便的操作
