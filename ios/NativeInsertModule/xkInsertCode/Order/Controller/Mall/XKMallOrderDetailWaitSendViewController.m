//
//  XKMallOrderDetailWaitSendViewController.m
//  XKSquare
//
...
#import "XKMallOrderDetailWaitSendInfoCell.h"
...
@interface XKMallOrderDetailWaitSendViewController () <UITableViewDelegate,UITableViewDataSource>
...
@implementation XKMallOrderDetailWaitSendViewController
...
- (void)moreBtnClick:(UIButton *)sender {
    XKWeakSelf(ws);
    XKMenuView *moreMenuVuew = [XKMenuView menuWithTitles:@[@"联系客服",@"退货"] images:nil width:80 relyonView:sender clickBlock:^(NSInteger index, NSString *text) {
        if(index == 0) {
            ...
            apply.status = OrderStatusWaitSend;
            [ws.navigationController pushViewController:apply animated:YES];
        }
    }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    switch (indexPath.section) {
        case 0:
        {
            ...
            [cell handleOrderDetailWithType:MOT_PRE_SHIP dataModel:_viewModel];
            return cell;
        }
            break;
        case 1:
        ...
        case 3:
        {
            XKMallOrderDetailWaitSendInfoCell *cell = [tableView dequeueReusableCellWithIdentifier:@"XKMallOrderDetailWaitSendInfoCell" forIndexPath:indexPath];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        XKMallOrderDetailGoodsItem * item = _viewModel.goodsInfo[indexPath.row];
        if (![item.itemStatus isEqualToString:@"NONE"]) {
...
- (UITableView *)tableView {
    if(!_tableView) {
        ...
        [_tableView registerClass:[XKMallOrderDetailWaitSendInfoCell class] forCellReuseIdentifier:@"XKMallOrderDetailWaitSendInfoCell"];
...
- (XKMallOrderListBottomView *)bottomView {
    XKWeakSelf(ws);
    if(!_bottomView) {
        _bottomView = [XKMallOrderListBottomView MallOrderListBottomViewWithType:MallOrderDetailBottomViewWaitSend];
        _bottomView.tipBtnBlock = ^(UIButton *sender) {
            [XKHudView showTipMessage:@"提醒成功，商家会尽快处理你的订单"];
        };
        
        _bottomView.moreBtnBlock = ^(UIButton *sender) {
              [ws moreBtnClickForDetail:sender withGoodsItem:ws.viewModel functionName:@[@"退货",@"联系客服"]];
        };
