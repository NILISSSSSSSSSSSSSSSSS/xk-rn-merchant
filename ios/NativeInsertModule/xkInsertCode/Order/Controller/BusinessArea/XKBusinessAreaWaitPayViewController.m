//
//  XKMainBusinessAreaWaitPayViewController.m
//  XKSquare
//
...
#import "XKBusinessAreaWaitPaySingleCell.h"
#import "XKBusinessAreaWaitPayManyCell.h"
#import "XKAlertView.h"
#import "XKSquareTradingAreaTool.h"
#import "XKBusinessAreaOrderListModel.h"


@interface XKBusinessAreaWaitPayViewController () <UITableViewDelegate,UITableViewDataSource>
...
@implementation XKBusinessAreaWaitPayViewController
...
- (void)requestDataWithTips:(BOOL)tip {
    ...
    [XKSquareTradingAreaTool tradingAreaOrderList:muDic orderType:MOT_PAY success:^(NSArray<AreaOrderListModel *> *list) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(item.goods.count > 1) {
        XKBusinessAreaWaitPayManyCell *cell = [tableView dequeueReusableCellWithIdentifier:@"XKBusinessAreaWaitPayManyCell" forIndexPath:indexPath];
        [cell bindData:item];
        ...
        XKBusinessAreaWaitPaySingleCell *cell = [tableView dequeueReusableCellWithIdentifier:@"XKBusinessAreaWaitPaySingleCell" forIndexPath:indexPath];
        [cell bindData:item];
        cell.payBtnBlock = ^(UIButton *sender,NSIndexPath *index) {
...
- (UITableView *)tableView {
    if(!_tableView) {
        ...
        [_tableView registerClass:[XKBusinessAreaWaitPaySingleCell class] forCellReuseIdentifier:@"XKBusinessAreaWaitPaySingleCell"];
        [_tableView registerClass:[XKBusinessAreaWaitPayManyCell class] forCellReuseIdentifier:@"XKBusinessAreaWaitPayManyCell"];
        XKWeakSelf(ws);
