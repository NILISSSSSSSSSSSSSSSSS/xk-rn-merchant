//
//  XKIMMessageMerchantCardContentView.m
//  XKSquare
//
...
@implementation XKIMMessageMerchantCardContentView
...
- (void)layoutSubviews {
    ...
    [self.dateLab mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.mas_equalTo(self.bgImgView).offset(-7.0 * ScreenScale);
    }];
...
- (void)refresh:(NIMMessageModel *)data {
    ...
    self.titleLab.attributedText = [NSAttributedString rz_colorfulConfer:^(RZColorfulConferrer *confer) {
        confer.text(cardTypeStr).font(XKMediumFont(17.0)).textColor(HEX_RGB(0xFFFFFF));
        confer.text(@" ");
        confer.text([NSString stringWithFormat:@"%.1f", card.cardDiscount / 100.0]).font(XKRegularFont(12.0)).textColor(HEX_RGB(0xFFFFFF));
        confer.text(@"折").font(XKRegularFont(8.0)).textColor(HEX_RGB(0xFFFFFF));
    }];
...
- (UIImageView *)bgImgView {
    if (!_bgImgView) {
        ...
        _bgImgView.image = IMG_NAME(@"xk_bg_terminal_card");
    }
    return _bgImgView;
}
