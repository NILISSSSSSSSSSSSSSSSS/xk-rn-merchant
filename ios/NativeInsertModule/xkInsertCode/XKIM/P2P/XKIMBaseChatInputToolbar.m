...
@interface XKIMBaseChatInputToolbar()<NIMGrowingTextViewDelegate>
...
@implementation XKIMBaseChatInputToolbar


- (instancetype)initWithFrame:(CGRect)frame{
  self = [super initWithFrame:frame];
  if (self) {
    [self setBackgroundColor:UIColorFromRGB(0xf5f5f7)];
    
    _voiceButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [_voiceButton setFrame:CGRectMake(0, 0, 30, 30)];
    [_voiceButton setBackgroundImage:[UIImage nim_imageInKit:@"xk_btn_IM_toolbar_voice"] forState:UIControlStateNormal];
    [_voiceButton setBackgroundImage:[UIImage nim_imageInKit:@"xk_btn_IM_toolbar_voice"] forState:UIControlStateHighlighted];
    
    
    _emoticonBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [_emoticonBtn setFrame:CGRectMake(0, 0, 30, 30)];
    [_emoticonBtn setBackgroundImage:[UIImage nim_imageInKit:@"xk_btn_IM_toolbar_emoji"] forState:UIControlStateNormal];
    [_emoticonBtn setBackgroundImage:[UIImage nim_imageInKit:@"xk_btn_IM_toolbar_emoji"] forState:UIControlStateHighlighted];
    
    _moreMediaBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [_moreMediaBtn setFrame:CGRectMake(0, 0, 30, 30)];
    [_moreMediaBtn setBackgroundImage:[UIImage nim_imageInKit:@"xk_btn_IM_toolbar_more"] forState:UIControlStateNormal];
    [_moreMediaBtn setBackgroundImage:[UIImage nim_imageInKit:@"xk_btn_IM_toolbar_more"] forState:UIControlStateHighlighted];
    
    _recordButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [_recordButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [_recordButton.titleLabel setFont:[UIFont systemFontOfSize:14.f]];
    [_recordButton setBackgroundImage:[[UIImage imageNamed:@"xk_btn_IM_toolbar_inputTxtFileBG"] resizableImageWithCapInsets:UIEdgeInsetsMake(15,80,15,80) resizingMode:UIImageResizingModeStretch] forState:UIControlStateNormal];
    _recordButton.exclusiveTouch = YES;
    [_recordButton sizeToFit];
    
    _inputTextBkgImage = [[UIImageView alloc] initWithFrame:CGRectZero];
    [_inputTextBkgImage setImage:[[UIImage imageNamed:@"xk_btn_IM_toolbar_inputTxtFileBG"] resizableImageWithCapInsets:UIEdgeInsetsMake(15,80,15,80) resizingMode:UIImageResizingModeStretch]];
...
- (CGSize)sizeThatFits:(CGSize)size
{
  ...
  if (self.status == XKIMInputStatusAudio) {
    viewHeight = 54.5;
  }else{
    //算出 TextView 的宽度
...
- (void)update:(XKIMInputStatus)status
{
  ...
  if (status == XKIMInputStatusText || status == XKIMInputStatusMore)
  {
  ...
  else if(status == XKIMInputStatusAudio)
  {
...
{
  [self.voiceButton setBackgroundImage:selected?[UIImage nim_imageInKit:@"xk_btn_IM_toolbar_voice"]:[UIImage nim_imageInKit:@"xk_btn_IM_toolbar_voice"] forState:UIControlStateNormal];
  [self.voiceButton setBackgroundImage:selected?[UIImage nim_imageInKit:@"xk_btn_IM_toolbar_voice"]:[UIImage nim_imageInKit:@"xk_btn_IM_toolbar_voice"] forState:UIControlStateHighlighted];
}

- (void)updateEmotAndTextBtnImages:(BOOL)selected
{
  [self.emoticonBtn setBackgroundImage:selected?[UIImage nim_imageInKit:@"xk_btn_IM_toolbar_emoji"]:[UIImage nim_imageInKit:@"xk_btn_IM_toolbar_emoji"] forState:UIControlStateNormal];
  [self.emoticonBtn setBackgroundImage:selected?[UIImage nim_imageInKit:@"xk_btn_IM_toolbar_emoji"]:[UIImage nim_imageInKit:@"xk_btn_IM_toolbar_emoji"] forState:UIControlStateHighlighted];
...
@end
@implementation XKIMBaseChatInputToolbar(InputText)
