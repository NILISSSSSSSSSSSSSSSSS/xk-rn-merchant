/*******************************************************************************
 # File        : XKDeleteCollectionViewCell.m
 # Project     : XKSquare
 # Author      : Lin Li
 # Created     : 2018/9/14
 # Corporation :  水木科技
...
#define CELL_BUTTON_HEIGHT self.contentView.bounds.size.height
#define CONTENT_WIDTH SCREEN_WIDTH - 20
@interface XKDeleteCollectionViewCell ()<UIScrollViewDelegate>
...
@implementation XKDeleteCollectionViewCell
...
- (UILabel*)sureDeleteView {
    if (!_sureDeleteView) {
        _sureDeleteView = [[UILabel alloc]initWithFrame:CGRectMake(CONTENT_WIDTH + DELETE_BUTTON_WIDTH, 0, DELETE_BUTTON_WIDTH + 40, CELL_BUTTON_HEIGHT)];
...
- (void)configDeleteView {
    ...
    label.text = @"删除";
    label.textColor = [UIColor whiteColor];
    label.center = CGPointMake(self.deleteView.width/2 - 20, self.deleteView.height/2);
    label.font = [UIFont systemFontOfSize:15];
...
- (void)configContentView {
    ...
    self.scrollContent.contentSize = CGSizeMake(CONTENT_WIDTH + DELETE_BUTTON_WIDTH, 109);
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if (scrollView.contentOffset.x<MIN_OFFSET_X)  {
        ...
        if (self.isSelectDelete) {
            [scrollView setContentOffset:CGPointMake(DELETE_BUTTON_WIDTH + 40, 0) animated:YES];
        }else{
...
- (void)showSureDelete {
    ...
    self.sureDeleteView.frame = CGRectMake(CONTENT_WIDTH + DELETE_BUTTON_WIDTH, 0, DELETE_BUTTON_WIDTH + 40, CELL_BUTTON_HEIGHT);
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        [self.scrollContent setContentOffset:CGPointMake(DELETE_BUTTON_WIDTH + 40, 0) animated:YES];
...
- (void)cellSelectAction {
    ...
    if (self.selectBlock) {
        self.selectBlock(self.collectionView, self.indexPath);
    }
}
@end
