...
#import "XKGiftForFriendController.h"
#import "XKFriendGiftCell.h"
#import "XKGiftBaseModel.h"

@interface XKGiftForFriendController ()

@end

@implementation XKGiftForFriendController
...
- (void)requestIsRefresh:(BOOL)isRefresh params:(NSMutableDictionary *)params complete:(void(^)(NSString *error,NSArray *array))complete {
    [HTTPClient getEncryptRequestWithURLString:@"user/ua/friendGiftQPage/1.0" timeoutInterval:20 parameters:params success:^(id responseObject) {
        NSDictionary *dic = [responseObject xk_jsonToDic];
        NSArray *arr = [NSArray yy_modelArrayWithClass:[XKGiftBaseModel class] json:dic[@"data"]];
...
- (UITableViewCell *)returnCellForIndexPath:(NSIndexPath *)indexPath tableView:(UITableView *)tableView {
    XKFriendGiftCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
    ...
    XKGiftBaseModel *model = self.dataArray[indexPath.row];
...
- (void)configMoreForTableView:(UITableView *)tableView {
    [tableView registerClass:[XKFriendGiftCell class] forCellReuseIdentifier:@"cell"];
}
