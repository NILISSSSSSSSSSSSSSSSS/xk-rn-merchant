//
//  XKConsumeCouponDetailViewController.m
//  XKSquare
//
...
@interface XKConsumeCouponDetailViewController ()
...
@implementation XKConsumeCouponDetailViewController
...
-(void)handleData{
    self.goodsDecLabel.text = _dataModel.goods;
    self.classDecLabel.text = _dataModel.category;
    self.creatTimeDecLabel.text = [XKTimeSeparateHelper backYMDHMSStringByStrigulaSegmentWithTimestampString:[NSString stringWithFormat:@"%ld",_dataModel.dateTime]];
    if ([_dataModel.consumerCoupontype isEqualToString:@"pay"]) {
        self.priceLabel.text = [NSString stringWithFormat:@"-%ld",_dataModel.Amount];
    }
    else{
        self.priceLabel.text = [NSString stringWithFormat:@"+%ld",_dataModel.Amount];
    }
...
- (void)cofigNavigationBar {
    [self setNavTitle:@"消费券" WithColor:[UIColor whiteColor]];
}
