...
@interface XKMineFocusViewModel ()
/**请求页数*/
...
@implementation XKMineFocusViewModel
...
- (void)requestFocus:(NSIndexPath *)indexPath complete:(void (^)(NSString *, id))complete {
    ...
    if (model.followRelation != XKRelationNoting) { // 已经关注
        [XKFriendshipManager requestNoFocus:model.userId complete:^(NSString *error, id data) {
            if (error) {
                complete(error,nil);
            } else {
                model.followRelation = XKRelationNoting;
                complete(nil,@"");
            }
        }];
    } else {
        [XKFriendshipManager requestFocus:model.userId complete:^(NSString *error, id data) {
            if (error) {
                ...
                model.followRelation = XKRelationOneWay;
                complete(nil,@"");
            }
        }];
    }
...
- (void)requestWithParams:(NSDictionary *)params block:(void(^)(NSString *error,id data))block {
    [HTTPClient postEncryptRequestWithURLString:@"im/ua/followSyncList/1.0" timeoutInterval:20 parameters:params success:^(id responseObject) {
