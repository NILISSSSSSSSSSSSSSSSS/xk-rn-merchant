/*******************************************************************************
 # File        : XKMineFansListController.m
 # Project     : XKSquare
 # Author      : Jamesholy
 # Created     : 2018/9/14
 # Corporation : 水木科技
...
@interface XKMineFansListController () <UITableViewDelegate, UITableViewDataSource>
/**tableView*/
@property(nonatomic, strong) UITableView *tableView;
/**<##>*/
@property(nonatomic, strong) XKEmptyPlaceView *emptyView;
/**viewModel*/
@property(nonatomic, strong) XKMineFansViewModel *viewModel;
/**<##>*/
@property(nonatomic, assign) BOOL isSeeOther;
@end

@implementation XKMineFansListController
...
- (void)createUI {
    ...
    if (self.rid) {
        if ([self.rid isEqualToString:[XKUserInfo getCurrentUserId]]) {
            [self setNavTitle:@"我的粉丝" WithColor:[UIColor whiteColor]];
        } else {
            [self setNavTitle:@"Ta的粉丝" WithColor:[UIColor whiteColor]];
            self.isSeeOther = YES;
        }
    } else {
        [self setNavTitle:@"我的粉丝" WithColor:[UIColor whiteColor]];
    ...
    [self.viewModel requestIsRefresh:refresh complete:^(NSString *error, NSArray *array) {
        ...
        if (error) {
            ...
            if (self.viewModel.dataArray.count == 0) {
                ...
                [self.emptyView showWithImgName:kEmptyPlaceImgName title:nil des:[self isMine]?@"您还没有粉丝哦" : @"Ta还没有粉丝哦" tapClick:nil];
...
- (XKMineFansViewModel *)viewModel {
    if (!_viewModel) {
        _viewModel = [[XKMineFansViewModel alloc] init];
