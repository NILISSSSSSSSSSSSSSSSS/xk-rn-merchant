...
@protocol RootChildControllerProtocol<NSObject>
/**第一次请求*/
- (void)requestFirst;
@end
...
/**
 允许“返回上一个界面”时显示之前界面的状态栏颜色，设置为YES时，会记录界面跳转时的导航栏颜色，在pop返回时设置回去。默认为NO，界面消失时时统一遵循设置成白色。
 */
@property(nonatomic, assign) BOOL returnOriginalStatusStyleWhenPop;
