...
@property(nonatomic, assign) BOOL hasApper;
/**<##>*/
@property(nonatomic, assign) UIStatusBarStyle lastPageBarStyle;
...
- (instancetype)init {
  self = [super init];
  if (self) {
    self.returnOriginalStatusStyleWhenPop = YES;
    if (self.returnOriginalStatusStyleWhenPop == YES) {
      self.lastPageBarStyle = [UIApplication sharedApplication].statusBarStyle;
    }
  }
  return self;
}
...
- (void)viewWillDisappear:(BOOL)animated {
  ...
  //fix bug ：从rn界面界面跳入原生界面时有些界面生命周期会调用多次 时状态栏颜色不受控制问题
  if (self.hasApper) {
    [UIApplication sharedApplication].statusBarStyle = UIStatusBarStyleLightContent;
    self.hasApper = NO;
  }
...
- (void)setGlobalFont:(UIView *)view {
  if ([view isKindOfClass:[UIButton class]]) {
    ...
    btn.titleLabel.font = XKMediumFont(btn.titleLabel.font.pointSize);
  }
}
...
- (UIView *)containView {
  if (!_containView) {
    ...
    _containView.backgroundColor = HEX_RGB(0xEEEEEE);
...
- (void)didMoveToParentViewController:(UIViewController*)parent{
  ...
  if(!parent){
    [self inner_didPopToPreviousController];
...
- (void)inner_didPopToPreviousController {
  NSLog(@"inner_didPopToPreviousController");
  if (self.returnOriginalStatusStyleWhenPop == YES) {
    [UIApplication sharedApplication].statusBarStyle = self.lastPageBarStyle;
  }
}
