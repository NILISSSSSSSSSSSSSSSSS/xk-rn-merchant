...
-(void)longPress:(UILongPressGestureRecognizer *)longPress
{
    switch (longPress.state) {
        case UIGestureRecognizerStateBegan:{
            ...
            self.mIndexPath = [self indexPathForRowAtPoint:point];
            //indexpath可能为空
            if (self.mIndexPath) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    self.moveCell = [self cellForRowAtIndexPath:self.mIndexPath];
        ...
        case UIGestureRecognizerStateChanged:{
            ...
            if (exchangeIndex) {
                ...
                [self moveRowAtIndexPath:self.mIndexPath toIndexPath:exchangeIndex];
                self.mIndexPath = exchangeIndex;
...
- (void)startAutoScroll{
    ...
    if (exchangePath) {
        ...
        [self moveRowAtIndexPath:self.mIndexPath toIndexPath:exchangePath];
        self.mIndexPath = exchangePath;
    }
...
-(void)updateDataWithIndexPath:(NSIndexPath *)moveIndexPath {
    ...
    if ([self nestedArrayCheck:self.dataArray]) {
        if (self.mIndexPath.section == moveIndexPath.section) {
            NSMutableArray *originalArray = self.dataArray[self.mIndexPath.section];

            [originalArray exchangeObjectAtIndex:self.mIndexPath.row withObjectAtIndex:moveIndexPath.row];

        }else{
            NSMutableArray *originalArray = self.dataArray[self.mIndexPath.section];
            NSMutableArray *removeArray = self.dataArray[moveIndexPath.section];
            NSString * obj = [originalArray objectAtIndex:self.mIndexPath.row];
            [removeArray insertObject:obj atIndex:moveIndexPath.row];
            [originalArray removeObjectAtIndex:self.mIndexPath.row];
        }
      
    }else{
        [self.dataArray exchangeObjectAtIndex:self.mIndexPath.row withObjectAtIndex:moveIndexPath.row];
    }
}
...
{
    return objc_getAssociatedObject(self, "mIndexPath");
}
-(void)setMIndexPath:(NSIndexPath *)mIndexPath
{
    objc_setAssociatedObject(self, "mIndexPath", mIndexPath, OBJC_ASSOCIATION_COPY_NONATOMIC);
